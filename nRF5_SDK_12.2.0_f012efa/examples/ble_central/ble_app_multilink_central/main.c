...
int main(void)
{
	...
	//simple_uart_config(RTS_PIN_NUMBER, TX_PIN_NUMBER, CTS_PIN_NUMBER, RX_PIN_NUMBER, HWFC);
		simple_uart_config(RTS_PIN_NUMBER,15, CTS_PIN_NUMBER,17, HWFC);
//		simple_uart_putstring((const uint8_t *)" \n\rplay pressed");
    ...
    bsp_board_led_on(CENTRAL_SCANNING_LED);
	//	if(cr)
	//	led_status_send_to_all(1);
		for (;;)
    {
			//power_manage();
			cr = simple_uart_get();
				if(cr==0x31)//driver up
				{	
				led_status_send_to_all(1);
				//led_status_send_to_all(0);	
				}	
				else if(cr==0X32)// driver down
				{
				led_status_send_to_all(2);
				//led_status_send_to_all(0);
				}
				else if(cr==0X33) //front passenger up
				{
				led_status_send_to_all(3);
				//led_status_send_to_all(0);
				}	
				else if(cr==0X34)//front passenger down
				{
				led_status_send_to_all(4);//
				//led_status_send_to_all(0);
				}
					else if(cr==0X35)// rear right up
				{
				led_status_send_to_all(5);//6
				//led_status_send_to_all(0);
				}
					else if(cr==0X36)// rear right down
				{
				led_status_send_to_all(6);//6
				//led_status_send_to_all(0);
				}
					else if(cr==0X37)// rear left up
				{
				led_status_send_to_all(7);//6
				//led_status_send_to_all(0);
				}
				else if(cr==0X38)// rear left down
				{
				led_status_send_to_all(8);//6
				//led_status_send_to_all(0);
				}
				else if(cr==0X39)// light's on
				{
				led_status_send_to_all(9);//6
				//led_status_send_to_all(0);
				}
				else if(cr==0X40)//light's off 
				{
				led_status_send_to_all(10);//6
				//led_status_send_to_all(0);
				}
				else if(cr==0X41)// butten release
				{
				led_status_send_to_all(0);//6
				//led_status_send_to_all(0);
				}
				else
				{
