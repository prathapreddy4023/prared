...
#define MAX_BUFFER_SIZE                  10                                             /**< Max number of data octets that can be enqueued per buffer */
...
typedef struct
{
    buffer_entry_t buffer[MAX_BUFFER_ENTRIES]; /**< Maximum number of entires that can enqueued in the list */
    uint8_t        rp;                         /**< Index to the read location */
    uint8_t        wp;                         /**< Index to write location */
    uint8_t        count;                      /**< Nummber of elements in the list */
}buffer_list_t;
 ...
 * @param[out] actual_len    Provides acutal length of Key Pattern transmitted, making buffering of rest possible if needed.
 * @return     NRF_SUCCESS on sucess, BLE_ERROR_NO_TX_BUFFERS in case tranmission could not be completed due to lack of
 *             transmission buffer or other error codes indicating reason for failure.
 *
 * @note       In case of BLE_ERROR_NO_TX_BUFFERS, remaining patter that could not be tramsitted can be enqueued
 *             \ref buffer_enqueue function.
 *             In case a pattern of 'cofFEe' is the p_key_pattern, with pattern_len as 6 and pattern_offset as 0,
 *             the notifications as observed on the peer side would be
 *             1>    'c', 'o', 'f', 'F', 'E', 'e'
 *             2>    -  , 'o', 'f', 'F', 'E', 'e'
 *             3>    -  ,   -, 'f', 'F', 'E', 'e'
 *             4>    -  ,   -,   -, 'F', 'E', 'e'
 ...
 *             6>    -  ,   -,   -,   -,   -, 'e'
 *             7>    -  ,   -,   -,   -,   -,  -
 *             Here, - referes to release, 'c' refers to the key character being transmitted.
 ...
 */
uint32_t send_key_scan_press_release
         (
              /* IN */  ble_hids_t    * p_instance,
              /* IN */  uint8_t       * p_key_pattern,
              /* IN */  uint16_t      pattern_len,
              /* IN */  uint16_t      pattern_offset,
              /* OUT */ uint16_t      * actual_len
         )
{
    uint32_t retval;
    uint16_t offset;
    ...
    // HID Report Desciptor enumertes an array size of 6, the pattern hence shall not be any longer than this.
    ASSERT ((INPUT_REPORT_KEYS_MAX_LEN - 2) >= pattern_len);
    ASSERT (NULL != p_key_pattern);
    ASSERT (0!= pattern_len);
    ...
    do
    {
        ...
        memcpy (data + 2 +offset,p_key_pattern+offset,data_len - offset);
        retval = ble_hids_inp_rep_send (p_instance,INPUT_REPORT_KEYS_INDEX,INPUT_REPORT_KEYS_MAX_LEN,data);
...
/**@brief Initializes the buffer queue used to key events that could not be transmitted
 *
 * @warning This handler is an example only. You need to analyze how you wish to buffer or buffer at all.
 *
 * @note    In case pf HID keyboard, a temporary buffering could be employed to handle scenarios where
 *          encryption is not yet enabled or therewas a momentary link loss or there were no Transmit buffers.
 */
void buffer_init (void)
{
    ...
    {
        BUFFER_INIT_ELEMENT (buffer_count);
    }
}

/**@brief Function to enqueue key scan patterns that could not be tranmisted either completely of partically.
 *
 * @warning This handler is an example only. You need to analyze how you wish to send the key release.
 *
 ...
 * @return     NRF_SUCCESS on sucess, else an error code indicating reason for failure.
 */
uint32_t buffer_enqueue
         (
             /* IN */ ble_hids_t             * p_hids,
             /* IN */ uint8_t                * p_key_pattern,
             /* IN */ uint16_t               pattern_len,
             /* IN */ uint16_t               offset
         )
{
    ...
    else
    {
        ...
        buffer_list.count++;
        buffer_list.wp++;
        if (MAX_BUFFER_ENTRIES == buffer_list.wp)
        {
            buffer_list.wp = 0;
        }
    }
...
/**@brief Function to dequeue key scan patterns that could not be tranmisted either completely of partically.
 *
 * @warning This handler is an example only. You need to analyze how you wish to send the key release.
 *
...
uint32_t buffer_dequeue(bool tx_flag)
{
    ...
    else
    {
        ...
        remove_element = true;
        element = &buffer_list.buffer[(buffer_list.rp)];
        if (tx_flag)
        {
           retval = send_key_scan_press_release
                    (
                        element->p_instance,
                        element->p_data,
                        element->data_len,
           ...
           if ((BLE_ERROR_NO_TX_BUFFERS == retval) && (actual_len < element->data_len))
           {
               // Transmission could not be completed, do not remove the entry, adjust next data to be transmitted
               element->data_offset = actual_len;
               remove_element = false;
           }
        }

        if(true == remove_element)
        {
            BUFFER_INIT_ELEMENT (buffer_list.rp);
            buffer_list.rp++;
            buffer_list.count--;
            if (MAX_BUFFER_ENTRIES == buffer_list.rp)
            {
                buffer_list.rp = 0;
            }
        }
    ...
    else
    {
        err_code = send_key_scan_press_release(&m_hids,
                                         ...
                                         key_pattern_len,
                                               0,
                                               &actual_len);
        if ((BLE_ERROR_NO_TX_BUFFERS == err_code) && (actual_len < key_pattern_len))
        {
            err_code = buffer_enqueue (&m_hids,p_key_pattern,key_pattern_len,actual_len);
            APP_ERROR_CHECK (err_code);
        }
    }
...
static void on_hids_evt(ble_hids_t * p_hids, ble_hids_evt_t *p_evt)
{
    switch (p_evt->evt_type)
    {
        ...
        case BLE_HIDS_EVT_NOTIF_ENABLED:
        {
            ...
            {
                // The notification of the report that was enabled by the master is not interesting
