                                                ...
                                                const ble_hrs_init_t * p_hrs_init)
{
    ...
    char_md.char_props.notify = 1;
	  char_md.char_props.read   = 1;  //ABP
		char_md.char_props.write  = 1;  //ABP
    ...
    attr_char_value.init_len     = MAX_HRM_LEN; //ABP //hrm_encode(p_hrs, INITIAL_VALUE_HRM, encoded_initial_hrm);
    attr_char_value.init_offs    = 0;
    attr_char_value.max_len      = MAX_HRM_LEN;
    attr_char_value.p_value      = 0;////ABP encoded_initial_hrm;
...
uint32_t ble_hrs_heart_rate_measurement_send(ble_hrs_t * p_hrs, uint16_t length, uint8_t *heart_rate)
{
    uint32_t err_code;
    ...
    if (p_hrs->conn_handle != BLE_CONN_HANDLE_INVALID)
    {
        ...
        hvx_params.p_data   = heart_rate;//encoded_hrm;
